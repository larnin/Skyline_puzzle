...
/////// Diffuse:
                ...
                float3 diffuseColor = lerp(_Diffuse_var.rgb,_BoutonActivated.rgb,(_BoutonMask_var.r*_Button));
                float3 diffuse = (directDiffuse + indirectDiffuse) * diffuseColor;
////// Emissive:
                float node_3278 = 0.0;
                float4 _Emissive_var = tex2D(_Emissive,TRANSFORM_TEX(i.uv0, _Emissive));
                float3 emissive = lerp(float3(node_3278,node_3278,node_3278),(lerp(lerp(_Diffuse_var.rgb,_BoutonActivated.rgb,(_BoutonMask_var.r*_Button)),_EmissiveColor.rgb,_EmissiveColorBool)*_Emissive_var.rgb),_EmissiveStrength);
/// Final Color:
